...
resume-safe HTTPS downloads, `load.py` / `convert.py` / `viz/` are the
analysis and preview layers, the `cli/` package exposes `umbra` (one
module per family of verbs, shared options in `cli/_shared.py`).
