...
### Changed
- **Deploy files live under `deploy/`.** `Dockerfile`, `Dockerfile.mcp`,
  `docker-entrypoint.sh`, `docker-compose.yml`, and `railway.toml` moved out of
  the repo root. Image build context stays the repository root
  (`docker build -f deploy/Dockerfile … .` / `docker compose -f
  deploy/docker-compose.yml up`); Railway's `dockerfilePath` is
