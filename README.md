...
index) and `describe_scene` (a SAR-literate structured reading of a scene —
`{summary, observed_features, confidence, caveats}` — needs a vision-model key)
tools; a `umbra://context` resource with the product-type table and search
...
keeping with the library's design, the server's core stays deterministic — it
searches, geocodes and renders; the `[ai]`-gated tools reach a model only at the
injectable edge (an embedding, or a validated, provenance-stamped scene reading),
never letting a model output become a coordinate, URL or filter. It even
