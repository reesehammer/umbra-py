...
def llms_full_txt() -> str:
    ...
    lines.append(
        ...
        "info`, `preflight`, `chips`, `describe`, `ask`, `watch`, `semantic "
        "search`, `embed similar`/`search`, and the render commands "
        "`change`/`timescan`/`swipe`/`gallery`/`map`/`stack`) "
        "-- see `docs/schemas/` for the shapes, one published contract per "
        "document."
    )
    lines.append("")
