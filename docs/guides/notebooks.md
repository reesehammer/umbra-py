...
| 07 | [`07_sicd_amplitude.ipynb`](https://github.com/reesehammer/umbra-py/blob/main/examples/07_sicd_amplitude.ipynb) | Extract **amplitude** from a complex SICD (`umbra convert` discards phase). |
...
Need the complex pixels themselves rather than a geocoded amplitude GeoTIFF?
[Complex products (SICD/CPHD)](complex-downstream.md) is the handoff:
index → search `--product SICD` / `CPHD` → size-check → download → stop.
