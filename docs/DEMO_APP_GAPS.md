...
>   and [`TODO.md`](../TODO.md). The G7 packaging story is now **half shipped**:
>   the Dockerfile + compose for one-command self-hosting of `umbra serve` (with
>   a first-boot index fetch and a `/healthz` probe) has landed; the remaining
>   G7 gap is the **GitHub Pages deploy of the static `umbra demo` /
>   `catalog.pmtiles` showcase**. Other optional polish: baking thumbnails/labels
>   into the *published* snapshot, and precomputed showcase swipe/change/timescan
