...
- **Known dead-code bug — ✅ fixed (this PR)**: `_classify_asset`'s
  `"tif" in name` check could never match an uppercased string
  (`models.py`), so a GeoTIFF identified only by its `.tif` key with no
  geotiff media type (the current href-less catalog generation) silently
  lost its classification and its primary GEC/CSI product vanished from
  `available_assets`. The probe is now `"TIF" in name`, with a regression
  test covering extension-only classification; the `TODO.md` entry is deleted.
...
| 8 | ✅ **Done.** `_classify_asset`'s extension probe is now `"TIF" in name` (was a lowercase `"tif"` against the upper-cased name — dead code), so a GeoTIFF identified only by its `.tif` key with no geotiff media type classifies again instead of silently dropping its GEC/CSI product; covered by a regression test and the `TODO.md` entry is deleted | `models.py`, `tests/test_models.py` | ~5 lines |
