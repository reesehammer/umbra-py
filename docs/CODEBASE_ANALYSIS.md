...
| # | Recommendation | Where | Effort |
|---|---|---|---|
| 11 | ✅ **Done.** `py.typed` shipped earlier; the CI type gate now closes the rest — a `type-check` job runs `mypy` (config in `[tool.mypy]`, `warn_unused_ignores` + `warn_redundant_casts` on) against a core `[dev]` install on every PR, so the shipped inline types are verified rather than promised. Landing it fixed 18 genuine type issues across 7 modules (incl. latent bugs: `date > None` in `search_live`, `isoformat()`/sort-key on `None` in `viz`, `.submit()` on a `None` executor in `serve`, `object`-typed search backends → `UmbraCatalog \| CatalogIndex`). `mypy`/`types-requests`/`types-defusedxml` added to `dev` | package + CI | small |
